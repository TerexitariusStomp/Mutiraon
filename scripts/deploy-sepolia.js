...
async function main() {
    ...
    if (deployerBalance === 0n) {
        console.log("⚠️  Deployer shows 0 ETH balance - attempting deployment anyway");
        console.log("    (This might be due to RPC caching or transaction still pending)\n");
    } else {
        console.log("✅ Deployer has balance for deployment!\n");
    }
    ...
    console.log("✅ Mutiraon deployed to:", stablecoin.address);
    ...
    await pot.deployed();
    console.log("✅ Pot deployed to:", pot.address);

    // Deploy Vow (Debt Accounting)
    const Vow = await hre.ethers.getContractFactory("Vow");
    const vow = await Vow.deploy(vat.address, hre.ethers.ZeroAddress, hre.ethers.ZeroAddress);
    await vow.deployed();
    ...
    for (let i = 0; i < tokenNames.length; i++) {
        ...
        console.log(`✅ ${tokenName}Join deployed to:`, tokenJoin.address);
    }
    ...
    for (let i = 0; i < tokenNames.length; i++) {
        ...
        console.log(`✅ ${tokenName}Clipper deployed to:`, clipper.address);
    }
    ...
    // Initialize DaiJoin as authorized
    const daiJoinWard = await stablecoin.wards(daiJoin.address);
    if (daiJoinWard.eq(1)) {
        console.log("✅ DaiJoin already authorized in StableCoin");
    } else {
        console.log("⚠️ DaiJoin not authorized in StableCoin - manual authorization required");
    }
