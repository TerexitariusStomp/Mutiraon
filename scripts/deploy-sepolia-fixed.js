...
async function main() {
    ...
    // Use the correct private key directly
    ...
    try {
        ...
        for (let i = 0; i < tokenNames.length; i++) {
            ...
            const tokenJoin = await GemJoin.deploy(await vat.getAddress(), hre.ethers.formatBytes32String(ilkName), tokenAddress);
        ...
        for (let i = 0; i < tokenNames.length; i++) {
            ...
            const clipper = await Clipper.deploy(await vat.getAddress(), await spot.getAddress(), await dog.getAddress(), hre.ethers.formatBytes32String(ilkName));
        ...
        for (let i = 0; i < tokenNames.length; i++) {
            ...
            // Set spot price feed
            await spot.file(ilkBytes, hre.ethers.formatBytes32String("pip"), await clipperContracts[tokenName].getAddress());
            ...
            await dog.file(ilkBytes, hre.ethers.formatBytes32String("clip"), await clipperContracts[tokenName].getAddress());
